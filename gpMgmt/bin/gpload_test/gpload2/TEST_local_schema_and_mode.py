...
def test_401_gpload_yaml_existing_external_schema():
    ...
    TestBase.psql_run(cmd='DROP SCHEMA IF EXISTS %s;' % schema,
                      dbname='reuse_gptest')
    TestBase.psql_run(cmd='CREATE SCHEMA %s;' % schema,
                      dbname='reuse_gptest')
...
def test_442_gpload_yaml_external_schema_name_special_char():
    ...
    for (c, escape) in spec_dict:
        ...
        append_raw_sql(
                test_num,
                """DROP TABLE IF EXISTS ext_schema_spec_char_test;""")
        append_raw_sql(
                test_num,
                """CREATE TABLE ext_schema_spec_char_test
...
''' function concat is not supported in gp5
